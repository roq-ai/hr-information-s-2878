...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createEmployee() {
    ...
    if (body?.vacation_request?.length > 0) {
      const create_vacation_request = body.vacation_request;
      body.vacation_request = {
        create: create_vacation_request,
      };
    } else {
      delete body.vacation_request;
    }
